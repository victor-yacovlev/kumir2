...
GccBackend::GccBackend(QObject *parent) :
    QObject(parent)
{
...
void GccBackend::setVerbose(bool v)
{
    b_verbose = v;
    m_process->setProcessChannelMode(v? QProcess::ForwardedChannels : QProcess::SeparateChannels);
}

void GccBackend::setTemporaryDir(const QString &path, bool autoclean)
{
    s_tempDir = path;
    b_cleanTempDir = autoclean;
}

QString GccBackend::generateExecuable(
        ...
        QIODevice *out)
{
    ...
    QString command = gccCommand()+" "+generateArgumentsLine(sources, outName, systemLibs, kumirLibs);
...
QString GccBackend::generateArgumentsLine(
        const QList<StringPair> &sources,
...
QString GccBackend::libraryPath()
{
#ifdef Q_OS_WIN32
...
QString GccBackend::debugSystemLib(const QString & n)
{
#ifdef QT_NO_DEBUG
    if (n.startsWith("Qt")) {
#ifdef Q_OS_WIN32
...
QString GccBackend::debugKumirLib(const QString &n)
{
#ifdef QT_NO_DEBUG
    return n;
#endif
...
QString GccBackend::execuableSuffix()
{
#ifdef Q_OS_WIN32
    return ".exe";
#endif
...
QString GccBackend::gccCommand()
{
#ifdef Q_OS_WIN32
    return QDir::cleanPath(QCoreApplication::applicationDirPath()+"/../mingw/bin/mingw32-gcc.exe");
#endif
    return "gcc";
}

QString GccBackend::tempDirName() const
{
...
bool GccBackend::saveFiles(const QList<StringPair> &files) const
{
    for (int i=0; i<files.size(); i++) {
