...
BreakHandler::BreakHandler(MiDebugger * engine)
  : m_syncTimerId(-1)
  , engine_(engine)
{
...
QModelIndex BreakHandler::index(int row, int col, const QModelIndex &parent) const
{
    ...
    if (id.isMinor())
        return QModelIndex();
    if (!id.isValid()) return QModelIndex();
...
QVariant BreakHandler::data(const QModelIndex &mi, int role) const
{
    ...
    if(it != m_storage.end()) return QVariant();
    const BreakpointParameters &data = it->data;
    const BreakpointResponse &response = it->response;
    ...
    if (id.isMinor()) {
        if(id.minorPart() <= it->subItems.size()) return QVariant();
...
MiDebugger *BreakHandler::engine(BreakpointModelId id) const
{
    ConstIterator it = m_storage.find(id);
...
void BreakHandler::setEngine(BreakpointModelId id, MiDebugger *value)
{
    Iterator it = m_storage.find(id);
...
{
    if (const MiDebugger *e = engine(id)) {
...
void BreakHandler::handleAlienBreakpoint(const BreakpointResponse &response, MiDebugger *engine)
{
...
void BreakHandler::timerEvent(QTimerEvent *event)
{
    ...
    m_syncTimerId = -1;
    engine_->synchronizeBreakpoints();
}
...
BreakpointModelIds BreakHandler::engineBreakpointIds(MiDebugger *engine) const
{
    BreakpointModelIds ids;
    ConstIterator it = m_storage.constBegin(), et = m_storage.constEnd();
    for ( ; it != et; ++it)
...
QStringList BreakHandler::engineBreakpointPaths(MiDebugger *engine) const
{
    QSet<QString> set;
    ConstIterator it = m_storage.constBegin(), et = m_storage.constEnd();
    for ( ; it != et; ++it) {
