...
#ifndef GDBMI_BREAKHANDLER_H
#define GDBMI_BREAKHANDLER_H
...
public:
    BreakHandler(MiDebugger * engine);
    ~BreakHandler();
...
//    void saveSessionData();
    ...
    void handleAlienBreakpoint(const BreakpointResponse &response, MiDebugger *engine);
    ...
    BreakpointModelIds engineBreakpointIds(MiDebugger *engine) const;
    BreakpointModelIds unclaimedBreakpointIds() const;
    int size() const { return m_storage.size(); }
    QStringList engineBreakpointPaths(MiDebugger *engine) const;
...
//        int lineNumber, bool useMarkerPosition = true);
    ...
    MiDebugger *engine(BreakpointModelId id) const;
    void setEngine(BreakpointModelId id, MiDebugger *engine);
...
private:
    ...
    Q_SLOT void changeLineNumberFromMarkerHelper(BreakpointModelId id, int lineNumber);
...
//        QIcon icon() const;
        ...
        MiDebugger *engine;  // Engine currently handling the breakpoint.
